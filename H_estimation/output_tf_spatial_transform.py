...
def Stitching_Domain_STN(inputs, H_tf, size, resized_shift):
    ...
    def _transform(image_tf, H_tf, width_max, width_min, height_max, height_min):
        ...
        # input_transformed = _interpolate(image_tf, x_s_flat, y_s_flat, (256, 256))
        output = tf.reshape(input_transformed, tf.stack([num_batch, out_height,  out_width, num_channels]))
        # output = tf.reshape(input_transformed, tf.stack([num_batch, 256, 256, num_channels]))
        return output
    ...
    img1_tf = inputs[... , 0:inputs.shape[3]//2]+1.
    img1_tf = _transform(img1_tf, H_one, width_max, width_min, height_max, height_min)
    
    #step 2.
    warp_tf = inputs[...,inputs.shape[3]//2:inputs.shape[3]]+1.
    warp_tf = _transform(warp_tf, H_tf, width_max, width_min, height_max, height_min)

    img1_tf = img1_tf - 1.
    warp_tf = warp_tf - 1.

    one = tf.ones_like(inputs[... , 0], dtype=tf.float32)
    mask1 = _transform(tf.expand_dims(one, 3), H_one, width_max, width_min, height_max, height_min)
    mask2 = _transform(tf.expand_dims(one, 3), H_tf, width_max, width_min, height_max, height_min)
    ...
    output = tf.concat([img1_tf, warp_tf], axis=3)
    return output
